...
// Creació del Vertex Array Object del terra
  ...
  glGenVertexArrays(1, &VAO_llum);
  glBindVertexArray(VAO_llum);
  glm::vec3 llum (0.0, 1.0, 2.0);
  glm::vec3 color (0.8, 0.8, 0.8);
  glm::vec3 llumPos[1] = {
      llum
  };
  glm::vec3 colorLlum[1] = {
      color
  };
  //llum i color
  glGenBuffers(1, &VBO_llumPos);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_llumPos);
  glBufferData(GL_ARRAY_BUFFER, sizeof(llumPos), llumPos, GL_STATIC_DRAW);

  glVertexAttribPointer(llumLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(llumLoc);

  glGenBuffers(1, &VBO_llumColor);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_llumColor);
  glBufferData(GL_ARRAY_BUFFER, sizeof(colorLlum), colorLlum, GL_STATIC_DRAW);

  glVertexAttribPointer(llumColorLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(llumColorLoc);
...
void MyGLWidget::carregaShaders ()
{
  ...
  //Demanem uniforms de llum
  llumColorLoc = glGetUniformLocation (program->programId(), "llumColor");
  llumLoc = glGetUniformLocation (program->programId(), "llumPos");
}
